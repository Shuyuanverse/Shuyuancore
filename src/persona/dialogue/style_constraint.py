...
class StyleConstraintEncoder:
    ...
    def encode(
        ...
        if "catchphrase_freq" in style_features:
            dimensions.append(ConstraintDimension(
    ...
    def merge_constraints(
        ...
        for dim_name, dims in dimension_map.items():
            ...
            # 加权平均 target_value
            target_values = [d.target_value for d in dims if isinstance(d.target_value, (int, float))]
            if target_values:
                avg_target = sum(t * w for t, w in zip(target_values, weights[:len(target_values)]))
            else:
