...
class CoordinatedResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "review_conflict_level": self.review_conflict_level.value
            if self.review_conflict_level
            else None,
            "correction_strategy": self.correction_strategy.value
            if self.correction_strategy
            else None,
            "total_rounds": self.total_rounds,
            ...
            if self.config.enable_review:
                for review_round in range(self.config.max_review_rounds):
                    ...
                    if review_task.status == TaskStatus.COMPLETED:
                        ...
                        # 判断冲突级别
                        conflict_level_str = review_output.get("overall_conflict_level", "none")
                                ...
                                if arb_task.status == TaskStatus.COMPLETED:
                                    ...
                                    if arb_output.get("success", False):
                                        ...
                                        context.arbitration_result = ArbitrationResult(
                                            ...
                                            original_response=review_output.get("response", ""),
                                            strategy_used=CorrectionStrategy(
                                                arb_output.get("strategy_used", "minimal")
                                            ),
                                            correction_count=arb_output.get("correction_count", 0),
                                            alignment_before=arb_output.get(
                                                "alignment_before", 0.0
                                            ),
                                            alignment_after=arb_output.get("alignment_after", 0.0),
                                            success=True,
                                        )
