...
class BidirectionalMapper:
    ...
    def map_vector_to_values(
        ...
        if len(style_vector) != self.style_dim:
            raise ValueError(f"Style vector dimension mismatch: expected {self.style_dim}, got {len(style_vector)}")
        if len(decision_vector) != self.decision_dim:
            raise ValueError(f"Decision vector dimension mismatch: expected {self.decision_dim}, got {len(decision_vector)}")
    ...
    def create_snapshot(self, vector: np.ndarray, values: Dict[str, float]) -> str:
        ...
        snapshot_id = hashlib.sha256(
            f"{time.time()}_{np.sum(vector):.6f}".encode()
        ).hexdigest()[:16]
