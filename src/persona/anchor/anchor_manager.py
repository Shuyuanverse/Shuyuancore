...
class WassersteinDriftDetector:
    ...
    def permutation_test(self, samples_a: List[float], samples_b: List[float], num_permutations: int = 1000) -> float:
        """排列检验计算 p 值。
    ...
    def check_and_update(
        ...
        if drift_level in (DriftLevel.MODERATE, DriftLevel.SIGNIFICANT, DriftLevel.CRITICAL):
            new_version = self.create_new_version(
                ...
                metadata={"trigger": "drift", "drift_level": drift_level.value, "distance": distance},
            )
...
class AnchorSimilarityService:
    ...
    def compute_anchor_similarity(
        ...
        """
        if anchor1.anchor_type == AnchorType.DECISION and anchor2.anchor_type == AnchorType.DECISION:
