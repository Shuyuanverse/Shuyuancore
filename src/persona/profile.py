...
class StyleDimensions:
    ...
    def to_vector(self) -> list[float]:
        return [self.colloquial, self.formal, self.emotional,
                self.interactive, self.logical, self.concise, self.expressive]

    def to_prompt_text(self) -> str:
        labels = {
            "口语化程度": self.colloquial, "正式程度": self.formal,
            "情感表达": self.emotional, "互动风格": self.interactive,
            "逻辑严谨": self.logical, "表达简洁": self.concise,
            "表现力": self.expressive,
        }
