...
class StyleVectorGenerator:
    ...
    DIMENSION_NAMES = [
        "vocab_richness", "vocab_size_norm", "hapax_ratio", "avg_word_length",
        "word_freq_entropy", "content_word_ratio", "function_word_ratio",
        "rare_word_ratio", "neologism_ratio", "loanword_ratio",
        "sentence_length_mean", "short_sentence_ratio", "long_sentence_ratio",
        "sentence_length_variance", "declarative_ratio", "interrogative_ratio",
        "exclamatory_ratio", "sentence_opening_diversity", "sentence_ending_diversity",
        "avg_clause_count",
        "period_ratio", "comma_ratio", "question_ratio", "exclamation_ratio",
        "colon_ratio", "semicolon_ratio", "dash_ratio", "ellipsis_ratio",
        "bracket_ratio", "punctuation_diversity",
        "syntactic_complexity", "clause_density", "subordinate_ratio",
        "coordination_ratio", "nesting_depth", "sentence_type_entropy",
        "pattern_consistency", "transition_word_ratio", "opening_pattern_strength",
        "ending_pattern_strength",
        "colloquial_score", "formal_score", "emotional_score",
        "interactive_score", "logical_score", "concise_score",
        "expressive_score", "style_consistency", "type_confidence",
        "overall_complexity",
        "complexity_index", "information_density", "coherence_score",
        "redundancy_ratio", "creativity_index", "emotional_valence",
        "engagement_score", "readability_score", "uniqueness_score",
        "style_stability",
    ]
    
    def generate(self, result: StyleExtractionResult, profile: StyleProfile | None = None) -> StyleVector:
        """从风格提取结果生成 60 维向量。
    ...
    def _build_vector(self, result: StyleExtractionResult, profile: StyleProfile | None) -> np.ndarray:
        """从提取结果和画像构建 60 维向量。
        ...
        if profile:
            vec[40] = profile.dimensions.get("colloquial", type("obj", (object,), {"value": 0})()).value if hasattr(profile.dimensions.get("colloquial", None), "value") else 0.5
            vec[41] = profile.dimensions.get("formal", type("obj", (object,), {"value": 0})()).value if hasattr(profile.dimensions.get("formal", None), "value") else 0.5
            vec[42] = profile.dimensions.get("emotional", type("obj", (object,), {"value": 0})()).value if hasattr(profile.dimensions.get("emotional", None), "value") else 0.5
            vec[43] = profile.dimensions.get("interactive", type("obj", (object,), {"value": 0})()).value if hasattr(profile.dimensions.get("interactive", None), "value") else 0.5
            vec[44] = profile.dimensions.get("logical", type("obj", (object,), {"value": 0})()).value if hasattr(profile.dimensions.get("logical", None), "value") else 0.5
            vec[45] = profile.dimensions.get("concise", type("obj", (object,), {"value": 0})()).value if hasattr(profile.dimensions.get("concise", None), "value") else 0.5
            vec[46] = profile.dimensions.get("expressive", type("obj", (object,), {"value": 0})()).value if hasattr(profile.dimensions.get("expressive", None), "value") else 0.5
            vec[47] = 0.7
    ...
    def compute_similarity(self, vec_a: np.ndarray, vec_b: np.ndarray, method: str = "cosine") -> Tuple[float, float]:
        """多指标相似度计算（余弦 + 欧氏）。
