...
class StyleEncoder:
    ...
    def _compute_dimensions(self, result: StyleExtractionResult) -> Dict[str, StyleDimension]:
        ...
        casual_punct = punct_habits.get("！", 0.0) + punct_habits.get("！", 0.0) + punct_habits.get("～", 0.0)
        colloquial = (
        ...
        emotional_catchphrases = sum(
            freq for phrase, freq in catchphrases.items()
        ...
        transition_words = sum(
            freq for phrase, freq in catchphrases.items()
            if any(p in phrase for p in ["然后", "接着", "另外", "不过", "但是", "而且", "所以", "因此"])
        )
        logical = (
            clause_ratio * 0.30 +
            long_ratio * 0.25 +
            comma_freq * 0.25 +
            transition_words * 0.20
        )
    ...
    def _calculate_confidence(self, dimensions: Dict[str, StyleDimension], style_type: str) -> float:
        """计算分类置信度。
