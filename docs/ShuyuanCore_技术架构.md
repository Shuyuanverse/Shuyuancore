...
• 三LLM分工：主/复盘/工具
## 二、核心模块设计
### 2.1 Agent 主循环（基于信念场架构）

ShuyuanCore 的 Agent 采用**信念场退化实现**，将对话历史视为信念的集合，Agent 持续维护信念存储，用户消息作为扰动更新信念场，读出器从场中提取相关信念生成回复。

#### 信念（Belief）
每条信念是一个结构化对象，包含内容、来源（user/assistant/tool/system）、置信度（初期固定 1.0）、时间戳、依赖关系等。

#### 核心组件
- **信念存储（BeliefStore）**：内存中按会话存储信念列表。
- **读出器（Reader）**：根据当前用户消息，从信念存储中提取最近 N 条信念，拼接成 LLM 上下文。
- **工具集成**：LLM 流式响应中实时检测 `tool_calls`，执行工具并将结果作为新信念写入存储。

#### 主循环流程
1. 用户输入消息 → 作为 `user` 信念写入存储。
2. 读出器获取上下文（最近 N 条信念）。
3. Agent 调用 LLM 流式生成回复，同时实时检测 `tool_calls`：
   - 若出现 `content` token，立即 `yield` 给用户。
   - 若出现 `tool_calls`，暂停输出，执行工具，将结果作为 `tool` 信念写入存储，然后跳回步骤 2 继续生成。
4. LLM 最终回复作为 `assistant` 信念写入存储。
5. 异步触发后台更新（空实现，预留）。

#### 设计原则
- 信念存储是退化的事实库，后续版本将支持置信度传播、依赖关系、持久化。
- 工具调用采用流式实时检测方案，零额外延迟。
### 2.2 六层记忆系统
**总览**：
|  |  |  |  |  |
| --- | --- | --- | --- | --- |
